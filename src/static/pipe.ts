...
export function pipe(...fns: UnknownConverter[]): UnknownConverter {
	...
	return <F extends UnknownColorSpace, T extends UnknownColorSpace>(
		...
		let acc: ColorSpaceData<UnknownColorSpace> = input
		for (const fn of fns) {
			acc = fn(acc)
		}
		return acc as T
