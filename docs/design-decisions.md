...
[Color.js] uses a tree for converting between colors with a "base" color that gets converted between. Separating out a more generic graph would be better for tree shaking since you don't need to include both the to and from conversions always.

See the [full colorspace graph].

[Color.js]: https://colorjs.io/docs/spaces
[full graph]: ./color-space-graph.md
...
After considering the options, I think I'm still going to go with the try/catch. It's not worth confusing people over a negligible performance increase and slightly more condensed code (see examples below).
...
```ts
import { convertAny as _convertAny } from "color-tools"
...
const convertAny = fromThrowable(_convertAny, true)
```
...
import { convertAny as _convertAny } from "color-"

type ParseError = { message: string }
const toParseError = (): ParseError => ({ message: "Parse Error" })
const convertAny = Result.fromThrowable(_convertAny, toParseError)
```
...
Pros: This is probably how most people expect errors to be handled.
